...
scoreboard_pipeline = ScoreboardPipeline('Development').run()
...
for scoreboard in scoreboard_data.iter_rows(named=True):    
    boxscore_pipeline = BoxscorePipeline(scoreboard, 'Development').run()
    boxscore_data = boxscore_pipeline['loaded']
    
    pbp_start_action = 0
    playbyplay_pipeline = PlayByPlayPipeline(scoreboard, boxscore_data, pbp_start_action, 'Development').run()
